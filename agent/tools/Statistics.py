...
def calc_single_image_hotspot_tif(file_path: str, threshold: float, output_path: str, uint8: bool = False) -> str:
    ...
    # Convert multi-band image to single-band (grayscale) by taking mean across bands
    if img.ndim == 3:
...
def subtract(img1_path: str, img2_path: str, output_path: str) -> str:
    ...
    try:
        ...
        # Resolve output_path against TEMP_DIR early so the suffix
        # check below works correctly (it is a string until this point).
        output_path = Path(TEMP_DIR) / output_path
        os.makedirs(output_path.parent, exist_ok=True)

        with rasterio.open(img1_path) as src:
            profile = src.profile
            # Determine band count from the result shape so the writer
            # matches multi-band (H, W, C) results as well as single-band
            # (H, W) ones. Without this, rasterio raises
            # "Source shape ... is inconsistent with given indexes 1".
            if result.ndim == 3:
                count = int(result.shape[2])
            else:
                count = 1
            profile.update(
            ...
            # Always force the GeoTIFF driver: the source profile's
            # driver (e.g. PNG, JPEG) cannot store Float32 and would
            # otherwise be inherited. This applies whether or not the
            # caller specified a file extension – rasterio falls back
            # to the source profile's driver when the output path has
            # no extension, which is the silent failure mode we hit.
            profile["driver"] = "GTiff"

            # If the user asked for a .png / .jpg / .jpeg output, swap
            # the extension to .tif so the on-disk file matches the
            # driver we just forced. The returned path reflects the
            # change so downstream tool calls keep working.
        ...
        # Return a self-describing error string so the agent can read
        # the message and try a different output path / driver combo
        # instead of receiving an opaque exception.
        return (
            f"Error in subtract: {type(exc).__name__}: {exc}. "
            "GeoTIFF (.tif) is required for Float32 output – use a "
            ".tif extension and try again."
        )
...
The path must point to a directory under one of the allowed roots:
  - the project working directory and its sub-directories
    (e.g. ``benchmark/data/question1``)
  - the MCP server's temporary output directory and its sub-directories
  - the system ``/tmp`` directory

Parameters:
    dir_path (str): Path to the directory. Absolute or relative paths
        are both accepted; relative paths are resolved against the
        project working directory.

Returns:
    list: Sorted list of file names in the directory (hidden files are
    skipped).

    If the path does not exist, is not a directory, is not readable, or
    is outside the allowed roots, a short error string is returned
    describing the problem and listing the valid roots so the agent can
    self-correct without raising an exception.
""")
def get_filelist(dir_path: str):
    """
    Returns a list of files in the specified directory, with path
    validation against an allow-list of roots.
    """
    import os
    from pathlib import Path

    # Resolve the input to an absolute, symlink-free path. Relative
    # inputs are interpreted relative to the current working directory,
    # which is the project root for both the eval scripts and the
    # Streamlit UI.
    try:
        resolved = Path(dir_path).expanduser().resolve(strict=False)
    except (OSError, RuntimeError) as exc:
        return (
            f"Error: cannot resolve path '{dir_path}': {exc}. "
            "Provide an existing directory path."
        )

    # Allow-list of roots the agent is permitted to introspect. The
    # MCP temp dir is computed the same way the tool scripts compute
    # theirs (TEMP_DIR is set from --temp_dir on the command line).
    try:
        project_root = Path(os.getcwd()).resolve()
    except OSError:
        project_root = resolved.anchor or Path("/")
    ...
    if not any(_is_within(resolved, root) for root in allowed_roots):
        ...
        return (
            f"Error: directory '{resolved}' is not within an allowed "
            f"root. Permitted roots are: {roots_str}. "
            "Use one of these roots (e.g. "
            f"'{project_root}/benchmark/data/question1') and try again."
        )

    if not resolved.exists():
        return (
            f"Error: directory '{resolved}' does not exist. Check the "
            "path and try again."
        )
    ...
    try:
        ...
        return (
            f"Error: permission denied while reading '{resolved}': "
            f"{exc}. Pick a directory the tool can access."
        )
    except OSError as exc:
        return f"Error: cannot list '{resolved}': {exc}."


def _is_within(path: Path, root: Path) -> bool:
    """Return True if `path` is the same as `root` or lives under it."""
    try:
