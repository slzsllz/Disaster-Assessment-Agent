...
An interactive UI for chatting with the agent is provided in `app.py`.
...
1. **Model selection** – every `agent/config_*.json` is listed in the sidebar; API key, base URL and model name are editable inline (defaults are read from `.env` via `config_utils.py`).
2. **System prompt** – pick a default, the learned *Enhanced* prompt from `training_free_results_*/earth_agent_practice_*_enhanced_config.json` (when one matches the selected model), or supply your own.
3. **Benchmark question loader** – the *Benchmark question* tab lists every entry of `benchmark/question.json`; clicking **Load into chat** pushes the question (plus its data directory and multiple-choice options) into the chat input.
4. **File uploads** – the *Upload data* tab stores files into a session-specific directory the agent can call `get_filelist` on; the path is shown to the user.
5. **Chat** – the *Chat* tab runs the LangChain ReAct agent over the existing MCP tool stack. By default only the final analysis / answer is displayed (no tool-call trace). An optional toggle in the sidebar reveals the per-step tool call/result log for debugging.

Output artefacts from the agent are written under `tmp/streamlit_out/<session-id>/out`, mirroring the path used by the existing `langchain_*.py` scripts.
